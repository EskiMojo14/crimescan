...
type WhitelistedState<Whitelist extends WhitelistDef> = {
  [Key in keyof RootState as Key extends keyof Whitelist ? Key : never]: Whitelist[Key] extends (keyof RootState[Key])[]
...
type InitialStates<Whitelist extends WhitelistDef> = {
  [Key in keyof RootState as Whitelist[Key] extends (keyof RootState[Key])[] | true ? Key : never]: RootState[Key];
};
...
const initialStates: InitialStates<PersistWhitelist> = { data, settings, locations };
...
export const hydrateState = (state: WhitelistedState<PersistWhitelist>): InitialStates<PersistWhitelist> =>
  objectFromEntries(
    objectEntries(persistWhitelist).map(([key, whitelist]) => [
      key,
      (Array.isArray(whitelist) ? { ...initialStates[key], ...state[key] } : state[key]) as RootState[typeof key],
 ...
 */
const createSubStateComparator = <K extends keyof PersistWhitelist>([key, whitelist]: [K, PersistWhitelist[K]]) =>
  createSelector(
