...
export const SkeletonBlock = <HTMLTag extends keyof JSX.IntrinsicElements = "div">({
  ...
  const finalContent =
    content ??
    (maxContentLength || minContentLength ? nanoid(randomInt(minContentLength, maxContentLength)) : undefined);
  return (
    ...
    >
      {finalContent && <span className={bemClasses("content")}>{finalContent}</span>}
    </Tag>
  );
};

export const SkeletonIcon = ({ className, height = 24, width = 24, ...props }: SkeletonBlockProps) => (
  <SkeletonBlock className={bemClasses({ extra: className, modifiers: "icon" })} {...{ height, width }} {...props} />
);
